...
function retiro_validar_solicitud(array $post): array {
    ...
    $condiciones = ['auto_elevador'=>retiro_texto($post['auto_elevador'] ?? '', 10), 'personal_tecmA'=>retiro_texto($post['personal_tecma'] ?? '', 10), 'personal_cantidad'=>retiro_texto($post['personal_cantidad'] ?? '', 20), 'electricidad'=>retiro_texto($post['electricidad'] ?? '', 10), 'electricidad_tipo'=>retiro_texto($post['electricidad_tipo'] ?? '', 20), 'electricidad_distancia'=>retiro_texto($post['electricidad_distancia'] ?? '', 20), 'observaciones'=>retiro_texto($post['condiciones_observaciones'] ?? '', 1000)];
...
function retiro_html_email(array $d, string $numero, string $fecha, bool $interno): string {
    ...
    $detalleInterno = $interno ? '<p><strong>Google Maps:</strong> ' . $maps . '</p><p><strong>Condiciones:</strong><br>' . nl2br(retiro_escape(implode(' | ', array_filter($d['condiciones'])))) . '</p><p><strong>Observaciones:</strong><br>' . nl2br(retiro_escape($d['observaciones'])) . '</p>' : '<p>Solicitud recibida no significa retiro programado.</p>';
